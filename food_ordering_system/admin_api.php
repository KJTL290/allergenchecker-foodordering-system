...
function isLogged() {
    return isset($_SESSION['admin_session']) || isset($_SESSION['kiosk_session']) || isset($_SESSION['ops_session']);
}
// --------------------
...
if ($action == 'toggle_stock') {
    // allow any logged-in staff (admin/kiosk/ops) to toggle stock from the operations UI
    if (!isLogged()) { echo json_encode(["status"=>"error","message"=>"unauthorized"]); exit; }
    $id = intval($_POST['id']); $status = intval($_POST['status']);
